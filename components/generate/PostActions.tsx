...
interface PostActionsProps {
  ...
  /** Post format from generator (single | story | landscape | portrait) for Instagram delivery */
  postFormat?: string | null
}

export default function PostActions({ postId, modelId, imageUrl, platform, caption, initialStatus, initialScheduledAt, isPlatformConnected, onSuccess, mediaType = 'image', postFormat = null }: PostActionsProps) {
          ...
          {!isPlatformConnected ? (
            ...
            <div className="space-y-4 pt-1">
              <p className="text-xs text-muted-foreground leading-relaxed">
                Connected accounts: <span className="font-medium text-foreground">{platformsArray.length}</span>. Choose publish
                now or pick a date, then use the button below.
              </p>
              ...
              <div className="flex items-center justify-between p-1 bg-muted rounded-xl border border-border dark:bg-white/5 dark:border-white/10">
                <button
                  ...
                  className={`flex-1 py-3 rounded-lg text-xs font-bold transition-all ${publishNow ? 'bg-primary text-primary-foreground shadow-md' : 'text-muted-foreground'}`}
                >
                  Publish now
                </button>
                <button
              ...
              >
                ...
                {publishNow ? `Publish now (${platformsArray.length} platforms)` : `Schedule for ${platformsArray.length} platforms`}
              </button>
            </div>
          )}
        </div>
